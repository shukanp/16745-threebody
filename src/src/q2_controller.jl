...
function lqr(A,B,Q,R; max_iters=2000, tol=1e-6)
    P = copy(Matrix(Q))
    n,m = size(B)
    K = zeros(m,n)
    K_prev = copy(K)
...
function gen_controller(nlp, Zref)
    ...
    # Get A & B matrices from X and U using dynamics_jacobians
    # TVLQR
    A = [zeros(n, n) for k = 1:N]
    B = [zeros(n, m) for k = 1:N]
    dt = nlp.times[2] - nlp.times[1]
    for k = 1:N
        t = nlp.times[k]
        A[k], B[k] = discrete_jacobian(nlp.model, Xref[k], Uref[k], t, dt)
    end
    ...
    K_Swing, P = tvlqr(A[1:N - 1], B[1: N - 1], Q, R, Qf)
    
    # END: MAKE SURE AT THE END TO SWITCH TO AN INF-HORIZON CONTROLLER
    # MAKE SURE TO LINEARIZE ABOUT EQUILIBRIUM POINT
    K_Stable = lqr(A[N-1], B[N-1], Q, R)#; max_iters = 2000)
    push!(K_Swing, K_Stable)

    # Return the Controller
    # ctrl IS SOME OBJECT given SOME STATE AND TIME, GIVE ME A CONTROL FOR THE SYSTEM
    ctrl = LQRController(K_Swing, Xref, Uref, nlp.times)
